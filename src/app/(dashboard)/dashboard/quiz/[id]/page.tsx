...
export default function QuizPage() {
  ...
  async function handleSubmit() {
    ...
    if (!trimmed) { setError('Please write your answer before submitting.'); return; }
    setError('');
    setSubmitting(true);
    try {
  ...
  // ── Loading state ────────────────────────────────────────────────────────────
  if (loading) {
    return <div className={styles.page}><p className={styles.stateMsg}>Loading question…</p></div>;
  }
      ...
      <div className={styles.page}>
        <div className={styles.topBar}>
          <nav className={styles.breadcrumb} aria-label="breadcrumb">
            <Link href="/dashboard/progress" className={styles.breadLink}>Question Bank</Link>
            <span className={styles.breadSep}>›</span>
            <span className={styles.breadCurrent}>Result</span>
          </nav>
        </div>
        <div className={styles.submittedBox}>
          <div className={`${styles.submittedIcon} ${total >= 50 ? styles.submittedIconPass : styles.submittedIconFail}`}>
            {total}%
          </div>
          <h2 className={styles.submittedTitle}>AI Grading Complete</h2>
          {result.aiFeedback && (
            <p className={styles.submittedSub}>{result.aiFeedback}</p>
          )}
          {result.scores && (
            <ScoreBreakdown scores={result.scores} />
          )}
          <div className={styles.submittedActions}>
            <Link href="/dashboard/progress" className={styles.backToBank}>Back to Question Bank</Link>
          </div>
        </div>
      </div>
    );
  ...
  if (!question) {
    return <div className={styles.page}><p className={styles.stateMsg}>{error || 'Question not found.'}</p></div>;
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.topBar}>
        <nav className={styles.breadcrumb} aria-label="breadcrumb">
          <Link href="/dashboard/reasoning" className={styles.breadLink}>Quizzes</Link>
          <span className={styles.breadSep}>›</span>
      ...
      <div className={styles.split}>
        ...
        <div className={styles.casePanel}>
          <div className={styles.caseCard}>
            {scenarioText && (
              <>
                <h2 className={styles.caseTitle}>Case Study: {question.subject ?? 'Legal Scenario'}</h2>
            ...
            <div className={styles.questionBox}>
              <p className={styles.questionText}>{question.prompt ?? question.text ?? 'Answer the question below.'}</p>
            </div>
        ...
        <div className={styles.workspacePanel}>
          ...
          <div className={styles.toolbar}>
            <button className={styles.toolBtn} onClick={() => handleFormat('bold')} title="Bold" type="button"><strong>B</strong></button>
            <button className={styles.toolBtn} onClick={() => handleFormat('italic')} title="Italic" type="button"><em>I</em></button>
            <button className={styles.toolBtn} onClick={() => handleFormat('list')} title="List" type="button">≡</button>
          </div>
          <textarea
            ref={textareaRef}
    ...
    <div className={styles.scoreBreakdown}>
      {rows.map(([label, val]) => (
        <div key={label} className={styles.scoreRow}>
          <span>{label}</span>
          <strong className={`${styles.scoreVal} ${val >= 20 ? styles.scoreValHigh : val >= 12 ? styles.scoreValMedium : styles.scoreValLow}`}>
            {val}/25
          </strong>
        </div>
      ))}

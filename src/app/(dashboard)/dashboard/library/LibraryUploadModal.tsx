...
import type { PresignedUrlRequest, PresignedUrlResponse, UploadCompleteRequest, ApiResponse } from '@/lib/types';
...
export default function LibraryUploadModal({ token, getUploadUrl, completeUpload, onClose, onSuccess }: Props) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!urlRes.data?.signedUrl) throw new Error('Failed to get upload URL');
      const { signedUrl, s3Key } = urlRes.data;

...
export default function ActivityHistoryModal({ data, onClose }: Props) {
  ...
  const filtered = activity.filter(item => {
    ...
    const matchesFilter = activeFilter === 'All' || (item.type ?? '').toLowerCase().includes(activeFilter.toLowerCase().replace(' ', '_'));
      ...
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.statsRow}>
          <div className={styles.statCard}>
            ...
            <p className={styles.statValue}>{weeklyHours} <span className={styles.statUnit}>hrs</span></p>
          </div>
          <div className={styles.statCard}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            ...
            <li key={i} className={styles.activityItem}>
              ...
              <div className={styles.activityBody}>
                ...
                <p className={styles.activityMeta}>
                  {item.createdAt ? `Viewed ${new Date(item.createdAt).toLocaleDateString()}` : 'Recently'} · {item.type ?? 'Activity'}
                </p>
              </div>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
